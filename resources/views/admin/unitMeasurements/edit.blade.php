...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.unit-measurements.update", [$unitMeasurement->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="unit_of_measurement">{{ trans('cruds.unitMeasurement.fields.unit_of_measurement') }}</label>
                    <input class="form-control {{ $errors->has('unit_of_measurement') ? 'is-invalid' : '' }}" type="text" name="unit_of_measurement" id="unit_of_measurement" value="{{ old('unit_of_measurement', $unitMeasurement->unit_of_measurement) }}" required>
