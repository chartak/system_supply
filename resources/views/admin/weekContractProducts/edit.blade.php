...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.weekContractProduct.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.week-contract-products.update", [$weekContractProduct->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('week_contract') ? 'is-invalid' : '' }}" name="week_contract_id" id="week_contract_id" required>
                    @foreach($week_contracts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('week_contract_id') ? old('week_contract_id') : $weekContractProduct->week_contract->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $weekContractProduct->quantity) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('year_con_prods') ? 'is-invalid' : '' }}" name="year_con_prods[]" id="year_con_prods" multiple required>
                    @foreach($year_con_prods as $id => $year_con_prod)
                        <option value="{{ $id }}" {{ (in_array($id, old('year_con_prods', [])) || $weekContractProduct->year_con_prods->contains($id)) ? 'selected' : '' }}>{{ $year_con_prod }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\WeekContractProduct::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $weekContractProduct->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
