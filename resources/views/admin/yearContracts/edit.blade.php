...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.yearContract.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.year-contracts.update", [$yearContract->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('customerid') ? 'is-invalid' : '' }}" name="customerid_id" id="customerid_id" required>
                        @foreach($customerids as $id => $entry)
                            <option value="{{ $id }}" {{ (old('customerid_id') ? old('customerid_id') : $yearContract->customerid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('supplierid') ? 'is-invalid' : '' }}" name="supplierid_id" id="supplierid_id" required>
                        @foreach($supplierids as $id => $entry)
                            <option value="{{ $id }}" {{ (old('supplierid_id') ? old('supplierid_id') : $yearContract->supplierid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $yearContract->start_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $yearContract->end_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                        ...
                        @foreach(App\Models\YearContract::STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('status', $yearContract->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
