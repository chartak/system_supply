...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.week-contracts.update", [$weekContract->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('year_contracts') ? 'is-invalid' : '' }}" name="year_contracts[]" id="year_contracts" multiple required>
                ...
                @if($errors->has('year_contracts'))
                    <span class="text-danger">{{ $errors->first('year_contracts') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $weekContract->start_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $weekContract->end_date) }}">
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('wk_con_file') ? 'is-invalid' : '' }}" id="wk_con_file-dropzone">
                </div>
                @if($errors->has('wk_con_file'))
                    <span class="text-danger">{{ $errors->first('wk_con_file') }}</span>
                @endif
...
Dropzone.options.wkConFileDropzone = {
    url: '{{ route('admin.week-contracts.storeMedia') }}',
    maxFilesize: 10, // MB
    addRemoveLinks: true,
    headers: {
