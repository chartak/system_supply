...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.weekContract.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.week-contracts.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('year_contracts') ? 'is-invalid' : '' }}" name="year_contracts[]" id="year_contracts" multiple required>
                    @foreach($year_contracts as $id => $year_contract)
                        <option value="{{ $id }}" {{ in_array($id, old('year_contracts', [])) ? 'selected' : '' }}>{{ $year_contract }}</option>
                    @endforeach
                </select>
